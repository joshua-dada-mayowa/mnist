...
# --- Model Loading ---
@st.cache_resource
def load_model(model_path="models/cvae_mnist_weights.weights.h5"):
    ...
    if os.path.exists(model_path):
        ...
        st.sidebar.warning("Model weights not found. Using an untrained model.")
        st.sidebar.info("To get better results, train the model first by running: `python train.py`")
    return model
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Train Model"):
            st.info("Training will take several minutes. Check the terminal for progress.")
            # Note: In a real deployment, you'd want to handle this differently
        ...
        if st.button(f"Generate 5 Images of '{digit_to_generate}'", use_container_width=True, type="primary"):
            with st.spinner("Generating..."):
                try:
                    ...
                    st.info("This might happen if the model isn't trained yet. Try training the model first.")

    except Exception as e:
        st.error(f"Error loading model: {str(e)}")
        st.info("Please ensure TensorFlow is properly installed and the model file exists.")
    ...
    with st.expander("Click here for Technical Details"):
        ...
        **Architecture:**
        - Encoder: Compresses input images and labels into a latent space
        - Decoder: Reconstructs images from latent representations and labels
        - Conditional: Uses digit labels to control generation
        """)
        st.code("""
